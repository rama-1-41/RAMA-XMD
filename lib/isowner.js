...
// Simple sudo check - can be extended
function isSudo(senderId) {
    if (!settings.sudoNumbers || settings.sudoNumbers.length === 0) return false;
    const sudoJids = settings.sudoNumbers.map(num => num + "@s.whatsapp.net");
    // Check exact match or if senderId contains the number
    return sudoJids.some(sudo => senderId === sudo || senderId.includes(sudo.split('@')[0]));
}
