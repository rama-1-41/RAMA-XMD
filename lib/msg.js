...
const sms = (conn, m, store) => {
    ...
    if (m.message) {
        ...
        // Check if m.msg exists and is an object before trying to access its properties
        if (m.msg && typeof m.msg === 'object') {
            quoted = (m.quoted = m.msg.contextInfo ? m.msg.contextInfo.quotedMessage : null);
            m.mentionedJid = m.msg.contextInfo ? m.msg.contextInfo.mentionedJid : [];
        } else {
            // If m.msg is not a valid object, ensure m.quoted is null
            m.quoted = null;
        }
        ...
        if (m.quoted) {
            ...
            if (quoted.viewOnceMessageV2) {
                ...
                m.quoted.mtype = type
                // Add an extra check here too to ensure contextInfo exists before accessing its properties
                if (m.msg && m.msg.contextInfo) {
                    ...
                    // Provide safe fallbacks if contextInfo is missing
                    m.quoted.id = null
    ...
    // Add a safety check for m.msg before accessing m.msg.url
    if (m.msg && m.msg.url) m.download = () => conn.downloadMediaMessage(m.msg)
    // Add a safety check for m.msg before trying to build m.text
    ...
    */
    ...
    m.senddoc = (doc, type, id = m.chat, option = {
        ...
        document: doc, mimetype: option.mimetype, fileName: option.filename, contextInfo: {
